...
#define TILE_W 128  // adjust for F0 vs F4
#define TILE_H 128
...
uint32_t width = 256;          // Current image width
uint32_t height = 256;         // Current image height
...
/* USER CODE BEGIN PFP */
uint64_t calculate_mandelbrot_fixed_point_arithmetic(int x_start, int y_start,
...
uint32_t estimated_ram_needed = width * height; // bytes (1 byte/pixel)
uint32_t available_ram = 128 * 1024; // STM32F4 SRAM in bytes, adjust for STM32F0
// Compute Mandelbrot with tiles
checksum = 0;
for (int y0 = 0; y0 < height; y0 += TILE_H) {
    for (int x0 = 0; x0 < width; x0 += TILE_W) {
        checksum += calculate_mandelbrot_fixed_point_arithmetic(
            x0, y0, TILE_W, TILE_H, width, height, MAX_ITER
        );
    }
}
...
// Turn on LED1 to signify end
HAL_GPIO_WritePin(GPIOB, GPIO_PIN_1, GPIO_PIN_SET);

// Hold LEDs for 1 second
HAL_Delay(1000);
                                         ...
                                         int max_iterations)
{
    ...
    for (int y = 0; y < tile_h; y++) {
        for (int x = 0; x < tile_w; x++) {
            ...
            if (global_x >= full_w || global_y >= full_h) continue;

            int64_t x0 = ((int64_t)global_x * (int64_t)(3.5 * FIXED_POINT_SCALE)) / full_w
                         - (int64_t)(2.5 * FIXED_POINT_SCALE);
            int64_t y0 = ((int64_t)global_y * (int64_t)(2.0 * FIXED_POINT_SCALE)) / full_h
                         - FIXED_POINT_SCALE;
            ...
            while (iteration < max_iterations) {
                ...
                if (xi2 + yi2 > ESCAPE_RADIUS_SQUARED) break;
